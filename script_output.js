...
const App = new Vue({
    ...
    data: {
        ...
        colors: ["orange", "cyan", "blue", "yellow", "purple"],
        designs: [
        ],
        selectedAtoms: [],
        showOffCanvas: false,
    ...
    computed: {
        ...
        visibleModels() {
            let vM = [];
            this.designs.forEach((element, index) => {
                if (element.visible) {
                    vM.push(index);
                }
            }
        ...
        mergedrepresentations() {
            if (this.loaded) {
                ...
                let merged = this.representations.concat(this.selectedRepresentations)

                // remove all representations that have model not visible
                let visibleModels = this.visibleModels
                merged = merged.filter((rep) => {
                    return visibleModels.includes(rep.model)
                })
                return merged
            }
            return {}
        ...
        sequence() {
            if (this.view != null) {
                ...
                //check if currentChain is in ModelChains
                let chain = ""
                if (this.modelChains.includes(this.currentChain)) {
                    chain = this.currentChain
                } else {
                    chain = this.modelChains[0]
                    this.currentChain = this.modelChains[0]
                }

                Object.keys(seq[this.currentModel][chain]).forEach((index) => {
                    residue = seq[this.currentModel][chain][index]
                    let key = ""
    ...
    methods: {
        ...
        designActiveClass(i, color) {
            if (color === "") {
                return "border-" + this.getColor(i) + "-200 border-l-" + this.getColor(i) + "-300  bg-gradient-to-r from-" + this.getColor(i) + "-100 to-transparent hover:from-" + this.getColor(i) + "-200"
            } else {
                return "border-" + color + "-200 border-l-" + color + "-300  bg-gradient-to-r from-" + color + "-100 to-transparent hover:from-" + color + "-200"
            }
        },
        getMoldata() {
            this.moldata = JSON.parse(document.getElementById("moldata").textContent);
        },
        getRepresentations() {
            this.representations = JSON.parse(document.getElementById("representations").textContent);
        },
        getDesigns() {
            this.designs = JSON.parse(document.getElementById("designs").textContent);
        },
        ...
        isModelVisible(model) {
            return this.designs[model].visible
        },
        ...
        selectModel(modelid) {
            this.designs[modelid].visible = !this.designs[modelid].visible
        },
    ...
    updated() {
        if (this.loaded) {
            ...
            this.dsModels.setSettings({
                selectables: document.getElementsByClassName('model-selectable'),
            })
    ...
    async mounted() {
        ...
        this.dsModels = new DragSelect({
            selectables: document.querySelectorAll('.model-selectable'),
            multiSelectMode: false,
            multiSelectToggling: false,
            area: this.$refs.modelSelectableArea,
            draggability: false
        });
        console.log(this.dsModels)
        var that = this;
        ...
        this.dsModels.subscribe('callback', (e) => {
            e.items.map((item) => {
                that.selectModel(parseInt(item.dataset.currentmodel))
            })
        });
